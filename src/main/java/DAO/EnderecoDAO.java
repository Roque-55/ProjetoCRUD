...
public class EnderecoDAO implements IDAO{
    ...
    public int alterar(EntidadeDominio entidade) {
        int status=0;
        String sql = "UPDATE PROJETO_CRUD.ENDERECO SET CEP=?, APELIDO=?, LOGRADOURO=?, NUMERO=?, BAIRRO=?, TIPO_ENDERECO=?, TIPO_LOGRADOURO=?, TIPO_RESIDENCIA=?, OBSERVACOES=?, CIDADE_ID=? WHERE id=?";
        Connection conn = null;
        PreparedStatement mysql = null;
        ...
        try {
            ...
            mysql.setString(2,((Endereco)entidade).getApelido());            
            mysql.setString(3,((Endereco)entidade).getLogradouro());
            mysql.setInt(4,((Endereco)entidade).getNumero());
            mysql.setString(5,((Endereco)entidade).getBairro());
            ...
            mysql.setInt(10,((Endereco)entidade).getCidade().getId());
            mysql.setInt(11,((Endereco)entidade).getId());
    ...
    public List<EntidadeDominio> consultar(EntidadeDominio entidade) {
        ...
        String sql = "SELECT *  FROM PROJETO_CRUD.ENDERECO a JOIN PROJETO_CRUD.CIDADE b ON a.CIDADE_ID = b.ID JOIN PROJETO_CRUD.ESTADO c ON b.ESTADO_ID = c.ID JOIN PROJETO_CRUD.PAIS d ON c.PAIS_ID = d.ID WHERE CLIENTE_ID = ? order by a.ID";
    ...
    public EntidadeDominio selecionar(EntidadeDominio entidade) {
        ...
        String sql = "\n" +
                "SELECT * FROM PROJETO_CRUD.ENDERECO a JOIN PROJETO_CRUD.CIDADE b ON a.CIDADE_ID = b.ID JOIN PROJETO_CRUD.ESTADO c ON b.ESTADO_ID = c.ID JOIN PROJETO_CRUD.PAIS d ON c.PAIS_ID = d.ID WHERE A.ID = ?";
        try {
            Connection conn = Conexao.getConexao();
            PreparedStatement mysql = conn.prepareStatement(sql);
            mysql.setInt(1,((Endereco) entidade).getId());
