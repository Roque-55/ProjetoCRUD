...
public class CartaoDAO implements IDAO{
    ...
    public String status(EntidadeDominio entidade) {
        int status = 0;
        String limpraPreferencial = "UPDATE PROJETO_CRUD.CARTAO c1\n" +
                "INNER JOIN PROJETO_CRUD.CARTAO c2 ON c1.CLIENTE_ID = c2.CLIENTE_ID\n" +
                "SET c1.PREFERENCIAL = FALSE\n" +
                "WHERE c2.ID = ?";
        String sql = "UPDATE PROJETO_CRUD.CARTAO SET PREFERENCIAL=TRUE WHERE ID=?";
        Connection conn = null;
        PreparedStatement mysql = null;
        ...
        try {
            ...
            mysql = conn.prepareStatement(limpraPreferencial);
            mysql.setInt(1,((Cartao) entidade).getId());
            mysql.executeUpdate();
            mysql = null;
